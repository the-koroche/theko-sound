...
class Java_java_lang_RuntimeException {
    private:
        ...
        Java_java_lang_RuntimeException(JNIEnv* env) {
    ...
    public:
        ~Java_java_lang_RuntimeException() {
            if (clazz) {
                bool attached = false;
        ...
        static Java_java_lang_RuntimeException* get(JNIEnv* env) {
            if (!env) return nullptr;
            if (!Java_java_lang_RuntimeException::jvm) {
                env->GetJavaVM(&Java_java_lang_RuntimeException::jvm);
            }
            static std::mutex mtx;
            static std::unique_ptr<Java_java_lang_RuntimeException> instance;
        
            std::lock_guard<std::mutex> lock(mtx);
            if (!instance || !instance->isValid()) {
                instance.reset(new Java_java_lang_RuntimeException(env));
            }
            return instance.get();
        }
        ...
        inline static jfieldID getfld__serialVersionUID(JNIEnv* env) {
            Java_java_lang_RuntimeException* self = get(env);
